...
| `selftest` | 任何人 | 41 条判据 → `evidence/` |

## 7. 报告固定格式（角色交活必须带这四行 + 一行成本）

```
做了什么：<一句话，别叙事>
证据：<evidence/ 或仓库里的路径（可带 sha256 前 12 位）>
判据：<跑了什么、多少条绿 / 关键读数>
依赖：<下一步等谁 / 等什么；没有就写"无">
本次任务：调用 N 次 · 输入 X · 输出 Y · 缓存读 Z · ≈¥W
```

**简洁（本人 2026-09-23 定）**：整条报告**不超过 15 行**；判据只留"数字 + 结论"，
原文一律进证据文件（对话里不贴）；一行一件事，不写过程叙事。
**成本用人民币**：`¥ = 美元估算 × cny_per_usd`（默认 **7.2**，存在 `talk.db` 的
`setting.cny_per_usd`，本人要改随时改）；取数照 ⑦ 的差值法（任务前后各读一次 `session_model_usage` 相减）。
