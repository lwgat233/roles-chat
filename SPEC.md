...
## 7. 报告固定格式（标题 + 四行 + 成本行，**整条 ≤8 行**｜2026-09-23 二次收紧）
...
```
- **首行必须是标题**：`【<项目>-<步号> <步名>】<角色全名> · <HH:MM>`（`report()` 会校验）。
- 每行一件事，「做」≤40 字；判据只留**数字 + 结论**；原始输出一律进证据文件（对话里不贴）。
- **超过 8 行 = 不合格**，`report()` 直接拒并说明超了几行；缺段也拒（缺哪段就报哪段）。
- 兼容旧写法（`做了什么 / 依赖 / 本次任务`），老报告照收。
- 成本用人民币：`≈¥W = 美元估算 × cny_per_usd`（默认 **7.2**，`talk.db` 的 `setting.cny_per_usd`）；
  取数照 ⑦ 差值法（任务前后各读一次 `session_model_usage` 相减）。
- 格式的权威出处是 `README.md`（本节与它保持一致）。
